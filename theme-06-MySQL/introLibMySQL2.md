...
// console.log(rows); // для контроля массива массивов
...
Чтобы программа работала добавим в наш модуль **utils.js**, созданный ранее, две зависимости, один метод и экспорт его:  
...
Функция **csv_to_json** использует синхронный метод для чтения файла, так как в этой лабораторке мы пока не делаем акцент на асинхронную работу для упрощения кода.  
Синхронный метода чтения csv-файла позволяет нам дождаться окончания его выполнения и потом все полученные данные отправить в базу данных.  
Метод csvjson.toObject(textCSV, { delimiter: ',' }) позволяет перевести полученный csv-файл в массив json-объектов. В качестве разделителя полей указали символ ','.  


Возвращаемся к основной программе - нам нужно перевести массив объектов в массив массивов значений, так как именно на такой формат настроен метод вставки множества записей: .query(query, [rows])  <-- вот тут второй параметр должен быть массивом массивов. Поэтому преобразуем json в массив rows, пробежавшись методом map по всем объектам:  
let rows = array.map(item => Object.values(item).slice(1));  
Тут мы из каждого объекта сделали массив его значений и убрали первый столбец (id) <-- так можно делат, если нам не нужно сохранять id и мы хотим, чтобы они автоматически назначались.  


так выглядит массив массивов, который передаём в метод query() в качестве второго параметра:  
```js
[
